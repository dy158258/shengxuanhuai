...
以盛宣怀与汉阳铁厂（1898—1908）为背景的互动叙事游戏。玩家扮演盛宣怀，在六个决策场景中做出选择，走向七种结局之一；也可以自由输入文字，向「AI 幕僚郑观应」呈报自己的决策，获得符合历史语境的反馈。
...
├── frontend/
│   ├── index.html       # 页面结构
│   ├── style.css        # 微信聊天式界面（米黄宣纸、仿宋字体、气泡+打字机动画）
│   └── script.js        # 游戏逻辑、存档、AI 输入
├── worker/
│   └── index.js         # Cloudflare Worker：AI 接口代理
├── data/
│   └── gameData.json    # 全部场景、选项、结局数据
└── README.md            # 本文件
```

---

## 〇、云服务器 VPS 部署（推荐 · 前后端一体 + AI 智能体）

> 如果你用自己的百度千帆「智能体」（AppBuilder）做 AI，推荐这套：一台服务器同时跑前端和后端，密钥存服务器，最省心。

### 前提
- 一台云服务器（百度 BCC / 阿里 ECS / 腾讯 CVM 均可，1核1G 足够）
- 服务器已装 **Node.js 18+**

### 步骤

1. **上传项目**：把整个目录（`frontend/`、`data/`、`server.js`、`package.json`、`index.html`、`.env`）传到服务器，如 `/home/你/sxh/`。
2. **安装依赖**：
   ```bash
   cd /home/你/sxh
   npm install
...
3. **配置密钥**：
   ```bash
   cp .env.example .env
   nano .env   # 填 BAIDU_APP_ID 和 BAIDU_API_KEY
   ```
4. **试跑**：`npm start`，看到 `已启动：http://localhost:3000` 即可。
5. **后台常驻**（生产推荐 pm2）：
   ```bash
   npm i -g pm2
   pm2 start server.js --name sxh
   pm2 save
   ```
6. **放行端口**：在云控制台「安全组」放行 3000（或改 80）；有域名则用 Nginx 反代到 3000。
7. 浏览器访问 `http://服务器IP:3000/` 即可玩。

### AI 智能体（意图匹配 + 盛宣怀口吻优化说辞）

前端玩家自由输入「决策意图」→ 后端 `POST /api/ai` → 调千帆 AppBuilder 智能体：
- 判断意图最接近哪个选项（A/B/C）
- 基于盛宣怀书信语料，生成符合盛宣怀口吻的「优化说辞」（文言 + 白话）

环境变量见 `.env.example`。未配置密钥时，前端自动降级为预设回应，游戏仍可玩。

> ⚠️ 千帆 AppBuilder 接口格式偶有变动。部署后若 AI 返回 500，把千帆智能体页「API 调用」里的 **curl 请求示例**发我，几分钟就能对齐。
...
> 项目根目录已带 `index.html` 跳转页，直接访问站点根会自动跳转到 `/frontend/`，可直接把站点根链接分享给玩家。
...
- **6 个决策场景**：场景一、二是「教学环节」，非历史路径可回退；场景三、四、五是「方向性抉择」，选定即锁定。
- **7 种结局**（A–G）：结局 A 为历史复刻者，B–G 为不同走向的推演结局。
- **选项标识**：★ 历史路径 / 🔄 可回退 / ⚠️ 不可回退。
- **AI 幕僚**：在场景下方文本框输入自己的决策，点「呈报」，郑观应（AI）会给出反馈。
